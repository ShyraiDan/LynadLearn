...
export default function SingleDefaultList({ params }: SingleDefaultListProps) {
  ...
  const {
    ...
    isLoading,
    error
  } = useSWR<ICollections>(`/api/collection/${collectionId}?activityType=flashcard`, fetcher, {
  ...
  const handlePin = async (item: ISection, e: MouseEvent<HTMLButtonElement>) => {
    ...
    if (session?.userId) {
      if (!bookmarkedItems.includes(item._id)) {
        const redirectLink = `/dashboard/flashcard/lists/${collectionId}/section/${item._id}`
        ...
        if (result.success) {
          ...
          mutate(`/api/collection/${collectionId}?activityType=flashcard`)
        } else {
                              ...
                              <div className={styles['accordion-details']}>
                                <div className={twMerge(styles['accordion-details-container'], 'dark:!bg-[#1D2D4D]')}>
                                  ...
                                  <div className={styles.info}>
                                    ...
                                    <NavigationLink
                                      href={`/dashboard/flashcard/list/${collectionId}/section/${section._id}`}
                                    >
